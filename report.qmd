...
|---|---|---|
| R.P.M. Vithanage | [ID] | Data cleaning, deduplication, BMI imputation, EDA |
| S.E.N. S. Silva | [ID] | Statistical testing, feature engineering, visualisations, modelling |
...
Stroke is a major global health problem and a leading cause of death and long-term disability. The World Health Organization estimates that approximately 15 million people experience a stroke each year, with roughly 5 million deaths and another 5 million people left permanently disabled. Stroke can cause paralysis, speech impairment, cognitive dysfunction, and vision loss, placing a significant and growing burden on healthcare systems worldwide.

Early identification of patients at high stroke risk allows clinicians to intervene before a cerebrovascular event occurs, reducing both clinical severity and long term rehabilitation costs. Despite advances in medical data collection, transforming large volumes of patient records into actionable risk predictions remains a central challenge in healthcare analytics.
...
Eleven interaction and transformation features were engineered prior to the train-test split, capturing clinically meaningful non-linear relationships that raw features cannot represent independently:

| Feature | Formula | Clinical Rationale |
|---|---|---|
| `age_squared` | age² | Non-linear acceleration of stroke risk with advancing age |
...
| `glucose_per_bmi` | glucose / (bmi + 1) | Metabolic efficiency ratio |
| `bmi_deviation` | \|bmi − 25\| | Distance from healthy BMI, capturing both under- and overweight risk |
| `log_glucose` | log(glucose + 1) | Normalises the right-skewed glucose distribution |
| `age_over_10` | age / 10 | Scaled age to reduce coefficient magnitude in linear models |
| `age_x_glucose` | age × glucose_level | Compounding metabolic-age interaction |
...
Forward fill and backward fill were considered but rejected — these are appropriate only for time-series data where adjacent observations are temporally related. Patient records in this dataset are independent.
...
Boxplots confirm the shift in age and glucose for the stroke class. BMI shows more overlap between groups but still contributes predictive signal — particularly through the `glucose_x_bmi` and `bmi_deviation` engineered features.
...
The statistical tests support the feature engineering strategy directly. Age is not only the strongest raw predictor but also benefits from non-linear transformation — `age_squared` achieves a higher correlation with the target than raw `age`. Cardiovascular burden works better as a combined indicator (`cvd_count`) than as isolated binary flags.
...
**Table 6.3: Top 10 XGBoost Feature Importance Scores**
...
| 9 | `age_over_10` | 0.058 | Engineered | Scaled age variant — complementary signal |
| 10 | `employment_type` | 0.055 | Original | Socioeconomic and stress-related proxy |
...
| Member Name | Student ID | Primary Contributions |
|---|---|---|
| R.P.M. Vithanage | [ID] | Data cleaning, deduplication, BMI imputation, EDA, statistical testing |
| S.E.N. S. Silva | [ID] | Feature engineering, model development, ensemble design, threshold tuning, visualisations, report writing |
